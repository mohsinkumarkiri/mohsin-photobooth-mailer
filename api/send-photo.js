...
// <body>
//   <center style="width:100%; background:#f2f2f2; padding:20px 0;">
//     <table width="600" class="container" style="max-width:600px; background:#fff; border-radius:10px; overflow:hidden; box-shadow:0 4px 10px rgba(0,0,.1);">
//       <tr>
//         <td class="header" style="background:#4CAF50; color:white; text-align:center; padding:30px 20px;">
//           <h1 style="margin:0; font-size:28px;">Lazulite Photo Booth</h1>
//         </td>
...
<style>
  ...
  .button {display:inline-block; padding:12px 25px; background:#4CAFFF; color:white; border-radius:6px; font-weight:bold; text-decoration:none;}
  @media only screen and (max-width:600px){
    .container {width:95% !important;}
    .header h1 {font-size:22px !important;}
    .content p {font-size:15px !important;}
...
<center style="width:100%; background:#f2f2f2; padding:20px 0;">
  <table width="600" class="container" style="max-width:600px; background:#fff; border-radius:10px; overflow:hidden; box-shadow:0 4px 10px rgba(0,0,0,0.1);">
    <tr>
      <td class="header" style="background:#007BFF; color:white; text-align:center; padding:20px;">
        <h1 style="margin:0; font-size:24px;">Lazulite Photo Booth</h1>
      </td>
    </tr>
    <tr>
      <td class="content" style="padding:20px; text-align:center;">
        <p>Hi there!</p>
        <p>Your memory is ready to enjoy:</p>
        {{IMAGE_SECTION}}
        {{VIDEO_SECTION}}
      </td>
    </tr>
...
export default async function handler(req, res) {
  ...
  if (imageData) {
    htmlContent = htmlContent.replace(
      ...
      `<img src="data:image/jpeg;base64,${imageData}" 
           alt="Your Photo" 
           style="width:100%; max-width:550px; height:auto; border-radius:10px; margin:15px 0;" />`
    );
  } else htmlContent = htmlContent.replace("{{IMAGE_SECTION}}", "");

  // --- Video Download Button ---
  if (videoUrl) {
    htmlContent = htmlContent.replace(
      "{{VIDEO_SECTION}}",
      `<a href="${videoUrl}" class="button" target="_blank" style="margin:20px 0; display:inline-block;">🎥 Download Your Video</a>`
    );
  } else htmlContent = htmlContent.replace("{{VIDEO_SECTION}}", "");

  try {
    await resend.emails.send({
      from: "Lazulite - Photo Booth <onboarding@resend.dev>",
      to: mailTo,
      subject: "Lazulite - Your Memory",
      html: htmlContent,
      attachments: [], // no attachment needed for inline image
    });

    console.log("✅ Email sent to:", mailTo);
    return res.status(200).json({ success: true, videoIncluded: !!videoUrl, imageIncluded: !!imageData });
  } catch (error) {
    console.error("❌ Email error:", error);
    return res.status(500).json({ error: "Email sending failed" });
  }
