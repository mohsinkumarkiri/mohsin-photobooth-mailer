...
export default async function handler(req, res) {
  ...
  const { mailTo, imageUrl, videoUrl } = req.body;

  if (!mailTo || (!imageUrl && !videoUrl)) {
    return res.status(400).json({
      error: "mailTo and either imageUrl or videoUrl are required",
    });
  }

  // Insert image if available (use URL, not Base64)
  let htmlContent = emailTemplate;
  if (imageUrl) {
    htmlContent = htmlContent.replace(
      "{{IMAGE_SECTION}}",
  ...
  // Insert video if available (thumbnail + button)
  if (videoUrl) {
    htmlContent = htmlContent.replace(
      "{{VIDEO_SECTION}}",
      `<div style="text-align:center; margin:20px 0;">
         <a href="${videoUrl}" class="button" target="_blank">▶️ Watch / Download Video</a>
       </div>`
    );
  } else {
    htmlContent = htmlContent.replace("{{VIDEO_SECTION}}", "");
  ...
  try {
    await resend.emails.send({
      ...
      subject: "Lazulite - Your Photo Booth Memories",
      html: htmlContent,
      attachments: [], // no Base64 attachments needed
    });
